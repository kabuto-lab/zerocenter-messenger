...
6. **Obfuscation transport (ScrambleStream) wired in Phase 4b — but only against pattern-matching DPI, not statistical analysis.** When `--obfs-key <32-byte hex>` is supplied, every byte on the TCP socket (Noise XX handshake included) is XOR'd with a ChaCha20 keystream, so a DPI box matching libp2p / Noise signatures sees only random bytes. Caveats remaining (Phase 4c candidates): the per-connection 12-byte nonce is sent in the clear (real Obfs4 derives it from an NTOR handshake — no plaintext prefix); no length-padding or inter-arrival-time jitter, so statistical-traffic analysis can still fingerprint ZeroCenter; `ScrambleStream::poll_write` keystream-advances by the full buffer length even on short inner writes. See INVARIANTS §17.
