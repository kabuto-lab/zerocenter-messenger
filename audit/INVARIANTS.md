...
**Dedup limitations.** Earlier drafts of this section claimed the ratchet's `try_skipped` cache "silently dedupes" duplicate deliveries via mailbox + outbox; audit F8 corrected that. In the current implementation `try_send_or_queue`'s offline branch enqueues the plaintext to BOTH the local outbox AND the mailbox publish path — and EACH path encrypts independently, producing two DIFFERENT ciphertexts at different chain positions (e.g. `header.n = 0` via mailbox, `header.n = 1` via direct delivery on reconnect). The recipient decrypts both successfully under different message keys and stores both as legitimate messages, surfacing as a visible duplicate in `history`. This is a known v0 UX bug; the proper fix (encrypt-once-then-deliver-both-paths) is a wire-format change to the outbox schema (store the encrypted bytes rather than the plaintext), deferred. AEAD-mediated dedup DOES work for identical-ciphertext duplicates (same Kad record fetched twice, or mailbox republish during a still-active direct connection); only the encrypt-twice case produces user-visible doubles.
