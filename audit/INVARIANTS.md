...
**Per-connection key + nonce (Phase 4c.1 — NTOR-style hidden handshake).** On connection open, each side generates an ephemeral X25519 keypair whose pubkey has an elligator2 `Randomized`-variant representative (retried up to 64 times against ~50% per-attempt success), sends the 32-byte representative on the wire, decodes the peer's 32-byte representative back to a Curve25519 pubkey, and runs X25519 DH to produce `shared_secret`. From `shared_secret || obfs_key` (concatenated, 64 bytes) we HKDF-SHA256 (salt = `"zerocenter-ntor-v1"`, info = `"chacha-key-nonce"`) a 44-byte OKM that splits into the per-connection `(chacha_key[32] || chacha_nonce[12])`. After the 32-byte exchange (each direction), every subsequent byte is scrambled with that ChaCha20 instance — Noise's XX handshake, Yamux frames, request-response payloads. The pre-shared `obfs_key` keeps its role as the authenticator: a MITM substituting their own ephemerals derives a different OKM and can't decrypt either side's scrambled stream.
...
**Known limitations.**
- `ScrambleStream::pending` is an unbounded `Vec<u8>`; a misbehaving inner that never accepts writes could grow it without limit. Bound at ~4×FRAME_QUANTUM and return `WouldBlock` past that — known-debt candidate (see `plans/ROADMAP.md`).
