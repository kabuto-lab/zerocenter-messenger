...
## §17. ScrambleStream wiring (Phase 4b + 4c.2 + 4c.2′ — shipped)
...
**Enforced at.**
- `src/network/scramble.rs::scramble_handshake` — nonce exchange + wrap; forwards `jitter_max_ms` into the resulting `ScrambleStream`.
...
- The 12-byte nonce header is in the clear. Real Obfs4 derives the nonce from an NTOR-like handshake so there is no plaintext prefix. Phase 4c.1 candidate (deferred — substantial scope).
